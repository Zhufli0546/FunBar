	...
	public void sendActivityEmail(List<Applicant> email, Activity activity) {
		...
		try {
			...
			for (Applicant a : email) {
				...
				if (a.getGender() == "女") {
					htmlCode += "<p style='text-align: center;font-size: 1.8em;color: rgb(22, 15, 1)'>"
							+ a.getApplicantName() + "小姐 您好</p>";
				} else if (a.getGender() == "男") {
					htmlCode += "<p style='text-align: center;font-size: 1.8em;color: rgb(22, 15, 1)'>"
							+ a.getApplicantName() + "先生 您好</p>";
				}

				htmlCode += "<p style='text-align: center;font-size: 1.5em;color: gray'>您報名的活動：</p>";
				htmlCode += "<p style='text-align: center;font-size: 2em;color:rgb(228, 121, 21)'>"
						+ activity.getEventName() + "</p>";
