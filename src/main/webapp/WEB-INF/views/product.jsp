...
<body>
	...
	<div class="page">
		...
		<div class="prodBlock">
			<a href="<c:url value='/ProductPicture/${pb.productId}'/>"
				class="prodImg jqzoom" style="width: 250px; height: 300px"
				title="${pb.productName}"> <img
				style="display: block; height: 300px; margin: auto;"
			...
			<div class="intro">
				...
				<div>
					<p style="font-size: 18px; padding: 10px 5px; color: #000">
						商品折扣: <span style="font-weight: bold; color: #CE0000;">${pb.discount}折</span>
					</p>
				</div>
				<div>
					<p style="font-size: 18px; padding: 10px 5px; color: #000">商品庫存:
						${pb.stock}</p>
				</div>
				...
				<div>
					...
					<div style="float: right; margin-right: 10px">
						<button type="button" class="btn button-add"
							data-product="${i.index}"
							style="margin: 10px 0; padding: 5px 10px; background-color: #019858; color: #fff; border-radius: 5px;">Add
							to Cart</button>
					</div>
				</div>
		...
		<div align="center" colspan="8">
			<a href="<c:url value='/shoppingCart?index=1' />"><button
					class="btn btn-back">Back to shop</button></a> <a
				href="<c:url value='/showCart' />"><button class="btn btn-info">Go
					to pay</button></a>
		</div>
							...
							$
									.ajax({
										url : "http://localhost:8080" + url
												+ "cart",
										data : {
											count : $(".selectCount").eq(index)
													.val(),
											productId : $(".pdid").eq(index)
													.val()
										},
										type : "POST",
										dataType : "JSON",
